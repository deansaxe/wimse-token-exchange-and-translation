...
* scope: OPTIONAL. A list of space-delimited, case-sensitive strings, as defined in [[RFC6749, Section 3.3](https://datatracker.ietf.org/doc/html/rfc6749#section-3.3)], that allow the client to specify the desired scope of the requested security token in the context of the service or resource where the token will be used.
* requested_token_type: `urn:ietf:params:oauth:token-type:access_token`
* subject_token: TBD. While this field is listed as REQUIRED for token exchange in [[RFC8693, Section 2.1](https://datatracker.ietf.org/doc/html/rfc8693#name-request)], in this case supplying the subject token again may be redundant, since it was already conveyed during the mTLS handshake. It is unclear whether repeating it here adds additional value. If supplied, it introduces the possibility that the client sends the wrong value, complicating client integration. It adds network overhead of a few KB, which could grow larger for longer certificate chains. It might be best to mark this as: OPTIONAL. The Workload's X.509 Certificate chain. The X.509 Certificate chain MUST chain to a previously-configured Trust Anchor certificate for the relying party, either directly or through one of the previously-configured Intermediate CA path-building certificates. This field MUST be expressed as a PEM-encoded certificate chain with newline characters removed.
