...
* subject_token: TBD. This is marked REQUIRED for token exchange, but in this case the subject token has already been conveyed during in mTLS handshake messages. It is unclear whether repeating it here adds any value or makes sense. If supplied, it introduces the possibility that the client sends the wrong value. It adds network overhead of a few KB, which could grow larger for longer certificate chains. It doesn't add additional security benefit. It would introduce additional client complexity and failure modes.
...
To mitigate the impact of Access Token theft, it is RECOMMENDED that the returned Access Token be sender-constrained. The Access Token SHOULD embed a hash of the X.509 Certificate used during the mTLS handshake to the Authorization Server. When the returned Access Token is used by clients, Resource Servers SHOULD verify that a matching certificate was used during the mTLS handshake. It is RECOMMENDED that the Access Token be bound to the X.509 Certificate that was used to obtain it, in the manner described in [RFC 8705 Section 3](https://datatracker.ietf.org/doc/html/rfc8705#name-mutual-tls-client-certifica). If the Access Token is a JWT, the X.509 Certificate hash can be represented in the "x5t#S256" member of the "cnf" payload claim. Authentication policy SHOULD be configurable to enable bound token enforcement as follows. Resource servers MAY enforce that an Access Token bound to an X.509 Certificate MUST NOT be used to access any protected resources, unless the same X.509 Certificate was used during the mTLS handshake to the Resource Server. 
